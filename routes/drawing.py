...
def handle_join_room_event(data):
    ...
    emit('join_room_announcement', {'user': user}, room=room)
    # Send drawing history to the newly joined user
    emit('draw_history', {'history': rooms_draw_history[room]}, room=request.sid)
...
#     leave_room(room)
#     emit('leave_room_announcement', {'user': user}, room=room)
...
def handle_drawing_event(data):
    ...
    emit('drawing', {'user': data['user'], 'drawing_data': drawing_data}, room=room)

@socketio.on('request_draw_history')
def handle_request_draw_history(data):
    room = data['room']
    sid = request.sid
    if room in rooms_draw_history:
        emit('draw_history', {'history': rooms_draw_history[room]}, room=sid)
...
def handle_start_event(data):
    ...
    emit('start_event', {'room': room, 'duration': duration}, room=room)

def end_event(room):
    emit('end_event', {'room': room}, room=room)
